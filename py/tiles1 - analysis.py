...
random_root = "c:/sim/turkexps/tiles/random"
ctrl_root = "c:/sim/turkexps/tiles/control"
...
r_trips = __builtin__.sum([out2trips(f) for f in glob.glob(random_root+'/*.out')],[])
c_trips = __builtin__.sum([out2trips(f) for f in glob.glob(ctrl_root+'/*.out')],[])


#(w,ux,uy)=
w=isotron.triples_sigmoid_isotron(r_trips,0.01,1000,c_trips)
#plt.plot(ux,uy)
#plt.show()
